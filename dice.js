...
function DieRoller(dice, emoji) {
  ...
  matches.forEach(function (match) {
    ...
    if (!successAxisRoll && diceConfig.types[type].successAxis) { 
      successAxisRoll = true; // If the symbol is on the success axis, then make sure to mark it in case there are 0 successes at the end
    }
  });
  ...
  var combined = ``;
  if(totals.S === 0 && totals.F === 0 && successAxisRoll) {
    combined += `0 Successes\n`;
  }
